...
import 'package:task_assignments/core/networking/exception.dart';
import 'package:task_assignments/core/repository/github_repository.dart';
...
import 'package:task_assignments/utils/alert_message.dart';
import 'package:task_assignments/widgets/loader_dialog.dart';
...
class GithubBloc with ChangeNotifier {
  ...
  set currentUserRepoList(List<Repositries> value) {
    _currentUserRepoList = value;
  }
...
//Get user repositries list
  Future<void> getUserRepositoriesList(String? username) async {
    try {
      setCurrentUserRepoListState(CurrentUserRepoListState.loading);
      List<Repositries>? _list = [];
      _list = (await _githubRepository.getUserRepositoriesList(username)).cast<Repositries>();
    ...
    try {
      ...
      if (actionUserListType == ActionUserListType.star ||
        ...
        _list = (await _githubRepository.getRepoActionUserList(
            username, actionUserListType, repoName)).cast<User>();
        _usersList = _list;
      } else {
        _list = (await _githubRepository.getActionUserList(
            username, actionUserListType)).cast<User>();
        _usersList = _list;
      }
